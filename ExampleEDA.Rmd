...
##my thoughts When you put "Value" as false you get the position of the string value in the set
grep("singlefav", names(selected_nzes2011), value = FALSE)

#my thoughts/ Here I created vector that contains "10" inside different places. Then I tried to see how the fuction behaves(as you can see "FALSE" for value returens the relative position of very value that contans "10" anywhere in the set)
k<- c(0,5,10,105,106,107,50)
grep(10, k,value = TRUE)
grep(10, k,value = FALSE)
...
## My thoughts/ I also decided to add sorting

selected_nzes2011 %>% group_by(jpartyvote) %>% summarise(count= n()) %>% arrange(desc(count))
```
...
selected_nzes2011 %>% filter(jpartyvote!="Don't know") %>% group_by(jpartyvote) %>% summarise(count= n()) %>% arrange(desc(count))
```
...
## My thougt/ does the position of "filter" function matter (IT DOES NOT MATTER)
selected_nzes2011 %>% filter(sameparty == "same") %>% 
  group_by(jpartyvote, X_singlefav, sameparty) %>%
  summarise(count = n()) 
...
## My thougt/ is it necessary to use %in% operator or jnzflike == "0"|jnzflike == "10" will suffice. ANSWR: YES it will work too
selected_nzes2011 %>% 
  filter(jnzflike == "0"|jnzflike == "10") %>%
  group_by(jnzflike) %>% 
  summarise(count = n())
...
## My thought/when you use "mutate" for the second time and add a column with the same name as already in the data frame, will the previous one be overwritten or there will be second column with the same name? ANSWER: yes it overrides
